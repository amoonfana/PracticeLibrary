...
Usage:		角色模型-带有深度描边
Author:		wangyang
History:	2015.07.14 创建
*/



Shader "Example/Character_Outline" {
	...
	Properties { 
		...
		_OutlineWidth ("描边宽度", Range (0.0, 0.06)) = .02 
    } 
		...
		inline half3 ComputeIllum(float2 uv, half4 c, half3 e)
		{
			...
			if (_IllumEnable > 0.0)
			{
				if (illum_r > IGNORE_GRAY)
				{
					...
					e.rgb += (c.rgb * illum_r * lerp(_IllumMin, _IllumMax, abs(_SinTime.w))) * _IllumPower;
				}
			}
			else
			{
